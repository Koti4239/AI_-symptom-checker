...
import sqlite3
import uuid
import webbrowser  # Added for opening URLs directly in the browser
...
def wiki_search(query):
    ...
    # Define a clean user-agent according to Wikipedia guidelines
    ...
    try:
        ...
        if page.exists():
            # Get summary snippet capped at 600 chars
            summary_snippet = (
                page.summary[:600] + "..."
...
# --------------------------
    ...
    if st.button("Predict"):
        if not symptoms.strip():
            ...
            # Dynamic prediction background context
            url, topic, wiki_description = wiki_search(prediction)
            if not wiki_description:
                wiki_description = "No static Wikipedia profile matches your exact evaluation prediction."
            ...
            # Display Wikipedia summary for prediction
            st.subheader("📚 Automated Diagnosis Reference")
...
# --------------------------
# MANUAL WIKIPEDIA SEARCH COMPONENT
# --------------------------
# This displays globally under your chosen pages to allow users to search up anything manually!
st.markdown("---")
st.subheader("📚 Wikipedia Search")

wiki_query = st.text_input("Search disease in Wikipedia")
...
st.markdown("---")
st.caption(
    "AI Symptom Checker | Streamlit + Live Wikipedia Dynamic Search Pipeline"
)
